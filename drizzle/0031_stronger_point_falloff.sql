WITH competitor_counts AS (
  ...
  SELECT
    ...
    s.score_value,
    c.ranking_direction,
    row_number() OVER board AS position,
...
SET points = round((c.pool * power(c.proximity, 3) * power(c.position::double precision, -0.15))::numeric, 2)::double precision
FROM calculated c
...
WITH target_ranked AS (
  ...
  FROM blocks
  GROUP BY user_id, top_points, tail_points
)
UPDATE users u
